// RegisterMode.js — Modal de modo registro
// Escanea 10 segundos y captura el dispositivo con RSSI más alto (más cercano)
...
export default function RegisterMode({ visible, manager, onDeviceFound, onClose }) {
  const [phase, setPhase] = useState('idle'); // idle | countdown | scanning | found | failed
  const [secondsLeft, setSecondsLeft] = useState(SCAN_SECONDS);
  const [candidates, setCandidates] = useState({}); // id → {rssi, name, samples}
  ...
  async function startScan() {
    ...
    manager.startDeviceScan(null, { allowDuplicates: true }, (error, device) => {
      if (error || !device || !scanRef.current) return;
      const rssi = device.rssi ?? -100;
      const id = device.id;
      if (!seen[id]) {
        seen[id] = { id, rssi, name: device.name || device.localName || null, samples: [rssi] };
      } else {
        seen[id].samples.push(rssi);
        // promedio de las últimas 5 muestras
        const s = seen[id].samples.slice(-5);
        seen[id].rssi = Math.round(s.reduce((a, b) => a + b, 0) / s.length);
        if (device.name && !seen[id].name) seen[id].name = device.name;
      }
      setCandidates({ ...seen });
    });
  ...
  // top 5 candidatos visibles durante el scan
  const topCandidates = Object.values(candidates)
          ...
          {phase === 'idle' && (
            <>
              <View style={styles.instructionBox}>
                <Text style={styles.instructionStep}>① Activa Bluetooth en su teléfono</Text>
                <Text style={styles.instructionStep}>② Ponlo a menos de 30 cm del tuyo</Text>
                <Text style={styles.instructionStep}>③ Presiona INICIAR ESCANEO</Text>
                <Text style={styles.instructionStep}>④ Confirma el dispositivo más cercano</Text>
              </View>
                ...
                <View style={styles.candidatesBox}>
                  ...
                  {topCandidates.map((dev, i) => (
                    <View key={dev.id} style={[styles.candidateRow, i === 0 && styles.candidateRowBest]}>
                      <Text style={[styles.candidateRank, i === 0 && { color: GREEN }]}>
                        {i === 0 ? '▶' : `${i + 1}`}
                      </Text>
                      <View style={{ flex: 1 }}>
                        <Text style={[styles.candidateName, i === 0 && { color: GREEN }]} numberOfLines={1}>
                          {dev.name || `DEV·${dev.id.slice(-4).toUpperCase()}`}
                        </Text>
                        <Text style={styles.candidateId} numberOfLines={1}>{dev.id.slice(-17)}</Text>
                      </View>
            ...
            <>
              <View style={styles.foundBox}>
                ...
                <Text style={styles.foundName}>
                  {bestDevice.name || `DEV·${bestDevice.id.slice(-4).toUpperCase()}`}
                </Text>
              ...
              {topCandidates.length > 1 && (
                <View style={[styles.candidatesBox, { marginTop: 10 }]}>
                  ...
                  {topCandidates.slice(1).map((dev) => (
                    <TouchableOpacity key={dev.id} style={styles.candidateRow}
                      onPress={() => { setBestDevice(dev); }}
                    >
                      <View style={{ flex: 1 }}>
          ...
          {phase === 'failed' && (
            <>
              <Text style={styles.failedText}>
                ...
                Asegúrate que Bluetooth esté activo en el otro teléfono y que esté cerca.
              </Text>
...
const styles = StyleSheet.create({
  ...
  instructionStep: { color: '#00aa33', fontSize: 12, fontFamily: MONO, lineHeight: 20 },
  ...
  candidateId: { color: '#003310', fontSize: 8, fontFamily: MONO },
